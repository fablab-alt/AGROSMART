import axios, { AxiosInstance, AxiosAdapter, InternalAxiosRequestConfig } from 'axios'
import { mockDispatch } from '@/lib/mocks'
...
// ---- Adaptateur mock visiteur ----
const visitorMockAdapter: AxiosAdapter = async (config) => {
  return mockDispatch(config)
}
...
function isVisitorMode(): boolean {
  ...
  try {
    ...
    return useAuthStore.getState().visitorMode === true
  } catch {
    return false
  }
}

// Intercepteur pour ajouter le token + court-circuiter en mode visiteur
api.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    // Si mode visiteur : utiliser l'adaptateur mock, aucun appel réseau réel
    if (isVisitorMode()) {
