...
// Visitor (demo) user — shown in UI when visitorMode === true
...
interface AuthState {
  ...
  enableVisitorMode: () => void
  disableVisitorMode: () => void
}
  ...
  persist(
    (set, get) => ({
      ...
      login: (user, token, refreshToken) => {
        // un login réel sort automatiquement du mode visiteur
        set({ user, token, refreshToken: refreshToken || null, isAuthenticated: true, isLoading: false, visitorMode: false })
      },
      logout: () => {
        if (typeof window !== 'undefined') {
          localStorage.removeItem('auth-storage')
        }
        set({ user: null, token: null, refreshToken: null, isAuthenticated: false, isLoading: false, visitorMode: false })
      },
    ...
    {
      name: 'auth-storage',
      // visitorMode est volontairement EXCLU : il ne doit jamais persister entre rafraîchissements
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state) {
          ...
          // Si on rehydrate avec le token visiteur, c'est qu'on a quitté la session — on nettoie
          if (state.token === VISITOR_TOKEN) {
            state.user = null
            state.token = null
